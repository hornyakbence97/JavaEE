...
<html>
    ...
    <body>
        <h1>Logged in: <c:out default="????" value="${sessionScope['user'].name}"> </c:out>     </h1>

                <form method="post" action="heroUpdate" style="border-style: solid;">
                    <h2>Update hero:</h2>
  <div class="form-group">
    <label for="name">Name of hero:</label>
    <input type="text" name="name" placeholder="Name of hero" class="form-control" readonly="readonly" value="<c:out default="????" value="${sessionScope['currentHero'].name}"></c:out>">
  </div>
  <div class="form-group">
    <label for="description">Description of hero:</label>
    <input type="text" name="description" placeholder="Description of hero" class="form-control" value="<c:out default="????" value="${sessionScope['currentHero'].description}"></c:out>">
  </div>
      <div class="form-group">
